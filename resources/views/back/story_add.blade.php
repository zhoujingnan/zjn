...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  <tr>
    <td width="99%" align="left" valign="top">您的位置：故事管理&nbsp;&nbsp;>&nbsp;&nbsp;添加故事</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <form action="{{url('/backstory/adddo')}}" method="post">
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
    	<input type="hidden" name="_token" value="{{csrf_token()}}">
      	<tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        	...
        	<td align="left" valign="middle" class="borderright borderbottom main-for">
        		<input type="text" name="story_title" value="" class="text-word">
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        	...
        	<td align="left" valign="middle" class="borderright borderbottom main-for">
        		<select name="member_id">
        			...
        			<option value="{{$val['member_id']}}">{{$val['member_name']}}</option>
        			@endforeach
        		</select>
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        	...
        	<td align="left" valign="middle" class="borderright borderbottom main-for">
        		<input type="text" name="story_desc" value="" class="text-word">
...
$(function(){
	...
	$("[name='story_title']").blur(function(){
		...
		if(story_title){
			$.ajax({
				type:'get',
				url:"<?php echo url('backstory/uniqueTitle')?>",
				data:{story_title:story_title},
				success:function(res){
					if(res==1){
						$(".s_story_title").html('<font color="red">用户名已存在</font>');
						name=false;
						return false;
					}else{
	...
	$("[name='member_id']").change(function(){
		...
		if(member_id){
			$(".s_member_id").html('<font color="green">√</font>');id=true;return true;
		}else{
			$(".s_member_id").html('<font color="red">请填写年龄</font>');id=false;return false;
		}
	})
	//判断简介
	$("[name='story_desc']").blur(function(){
		var story_desc=$(this).val();
		if(story_desc){
			$(".s_story_desc").html('<font color="green">√</font>');desc=true;return true;
		}else{
			$(".s_story_desc").html('<font color="red">请填写工作</font>');desc=false;return false;
		}
	})	
	//判断提交按钮
	$("form").submit(function(){
		if(name==true&&id==true&&desc==true){
			return true;
		}else{
			return false;
		}
