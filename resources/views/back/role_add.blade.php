...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  ...
  <tr>
    ...
    <form action="{{url('/backrole/adddo')}}" enctype="multipart/form-data" method="post">
...
$(function(){
	var name=false;
	$("[name='role_name']").trigger('focus');
	//判断会员名
	$("[name='role_name']").blur(function(){
		...
		if(role_name){
			$.ajax({
				...
				success:function(res){
					if(res==1){
						$(".s_role_name").html('<font color="red">用户名已存在</font>');
						name=false;
						return false;
					}else{
						$(".s_role_name").html('<font color="green">√</font>');
						name=true;return true;
					}
				}
			})
		}else{
			$(".s_role_name").html('<font color="red">请填写课堂名</font>');
